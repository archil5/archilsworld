...
const codePieces: CodePiece[] = [
  ...
  { id: 2, code: '<body style="background:#0f172a">', placed: false, order: 2 },
      ...
      <div className="w-full max-w-5xl flex flex-col gap-4">
        <div className="flex gap-2 justify-center items-center">
          {(["puzzle", "skills", "value"] as const).map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              ...
              style={{
                background: activeTab === tab ? "rgba(228,77,38,0.15)" : "rgba(15,23,42,0.6)",
                color: activeTab === tab ? "#E44D26" : "rgba(148,163,184,0.5)",
                border: `1px solid ${activeTab === tab ? "rgba(228,77,38,0.3)" : "rgba(148,163,184,0.1)"}`,
              }}>
        ...
        {activeTab === "puzzle" && (
          <div className="flex flex-col lg:flex-row gap-6 h-[450px]">
            <div className="flex-1 flex flex-col gap-3">
              <div className="flex-1 rounded-lg p-4 overflow-y-auto"
                ...
                <div className="flex items-center gap-2 mb-3">
                  <span className="text-xs font-mono" style={{ color: "#475569" }}>index.html</span>
                  <span className="text-[10px] font-mono ml-auto" style={{ color: "rgba(148,163,184,0.4)" }}>
                ...
                {placed.map((piece, i) => (
                  <motion.div key={piece.id} initial={{ opacity: 0, x: -20, scale: 0.9 }}
                    ...
                    <span className="w-6 text-right mr-3 select-none" style={{ color: "#475569" }}>{i + 1}</span>
                ...
                {placed.length < codePieces.length && (
                  <motion.div className="font-mono text-xs py-1 flex items-center"
                    ...
                    <span className="w-6 text-right mr-3" style={{ color: "#475569" }}>{placed.length + 1}</span>
                    <span style={{ color: "rgba(228,77,38,0.3)" }}>← place next piece here</span>
                  </motion.div>
                )}
              </div>
                ...
                {available.map(piece => {
                  ...
                  return (
                    <motion.button key={piece.id} onClick={() => handlePlace(piece)}
                      ...
                      animate={isNext ? { boxShadow: ["0 0 0px rgba(228,77,38,0)", "0 0 15px rgba(228,77,38,0.3)", "0 0 0px rgba(228,77,38,0)"] } : {}}
                      transition={isNext ? { repeat: Infinity, duration: 2 } : {}}
                      style={{
                        background: "rgba(15,23,42,0.8)",
                        color: isNext ? "#E44D26" : "rgba(228,77,38,0.5)",
                        border: `1px solid ${isNext ? "rgba(228,77,38,0.4)" : "rgba(228,77,38,0.1)"}`,
                      }}>
            ...
            <div className="flex-1 flex flex-col gap-3">
              <div className="flex-1 rounded-lg p-6 flex flex-col justify-center"
                ...
                <AnimatePresence mode="wait">
                  {showPreview ? (
                    <motion.div key="preview" initial={{ opacity: 0, rotateY: 90 }}
                      ...
                      <p className="text-lg mb-2" style={{ color: "#0078D4" }}>I made this.</p>
                      <p className="mb-4" style={{ color: "#e2e8f0" }}>It's ugly. It's wonderful. And it's <strong style={{ color: "#FFD700" }}>mine</strong>.</p>
                      <div className="mt-4 p-3 rounded-lg" style={{ background: "rgba(228,77,38,0.08)", border: "1px solid rgba(228,77,38,0.2)" }}>
                        <p className="text-sm font-body italic" style={{ color: "rgba(226,232,240,0.8)" }}>
                    ...
                    <motion.div key="progress" className="text-center">
                      <p className="font-body text-sm italic" style={{ color: "rgba(148,163,184,0.5)" }}>
          ...
          <motion.div className="grid grid-cols-2 md:grid-cols-3 gap-3 max-w-2xl mx-auto"
            ...
            {skillsLearned.map((skill, i) => (
              <motion.div key={skill} className="flex items-center gap-2 px-4 py-3 rounded-lg"
                ...
                style={{ background: "rgba(228,77,38,0.08)", border: "1px solid rgba(228,77,38,0.2)" }}>
          ...
          <motion.div className="max-w-lg mx-auto space-y-3" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            {valueDelivered.map((val, i) => (
              <motion.div key={i} className="flex items-start gap-3 px-4 py-3 rounded-lg"
                ...
                <span className="text-sm font-body" style={{ color: "rgba(226,232,240,0.8)" }}>{val}</span>
              </motion.div>
            ))}
          </motion.div>
        )}
